...
def teachers_list(request):
    teachers = Teacher.objects.all() # استرجاع جميع المعلمين من قاعدة البيانات
    context = {
        'teachers': teachers
    }
    return render(request, 'page/teachers_list.html', context)
def teacher_add(request):
    if request.method == 'POST':
        form = TeacherForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('page:teachers_list')
    else:
        form = TeacherForm()
    return render(request, 'page/teacher_form.html', {'form': form, 'action': 'Add'})
...
def teacher_edit(request, pk):
    ...
    return render(request, 'page/teacher_form.html', {'form': form, 'action': 'Edit'})
...
def teacher_delete(request, pk):
    ...
    if request.method == 'POST':
        teacher.delete()
        return redirect('page:teachers_list')
    return render(request, 'page/teacher_confirm_delete.html', {'teacher': teacher})
